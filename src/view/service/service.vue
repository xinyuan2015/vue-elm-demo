<template>
  <div class="rating-page">
    ...
    <section class="service-connect">
      <a href="http://ecs-im.ele.me" class="service-left">
        <svg>
          <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#human"></use>
        </svg>
        <span>在线客服</span>
      </a>
      <a href="tel:10105757" class="service-right">
        <svg>
    ...
    <section class="hot-questions" v-if="serviceData">
      <h4 class="question-header">热门问题</h4>
      <ul>
        <li v-for="(item, index) in questionTitle" :key="index" class="question-title" @click="toQuestionDetail(item, index)">
          <span>{{item}}</span>
